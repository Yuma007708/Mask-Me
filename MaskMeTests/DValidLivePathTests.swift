...
/// （480px 幅縮小）+ IMAGE モードスキャナーという「本スキャンより軽い経路」で
/// 検出される。本スキャン（フル解像度 VIDEO モード + enhance/ROI/flow）を測る
...
///   - bodyFP: bbox 中心が画面下部（cy > 0.6）かつ体級サイズ（高さ > 0.5）の
///     検出数。体誤検知の代理指標。顔アップ動画（顔だけで高さ 0.5 超）を
///     誤カウントしないよう、サイズ条件だけでなく位置条件も併用する。
final class DValidLivePathTests: XCTestCase {
    ...
    private func measureLivePath(name: String, url: URL) async throws -> LiveMetrics {
        ...
        while t <= duration {
            autoreleasepool {
                ...
                let faces = scanner.allLandmarks(in: UIImage(cgImage: downscaleTo480(cg)))
                metrics.frames += 1
                if !faces.isEmpty { metrics.liveHits += 1 }
                metrics.bodyFP += faces.filter {
                    $0.boundingBox.midY > 0.6 && $0.boundingBox.height > 0.5
                }.count
    ...
    /// MosaicPreviewController.detectionCGImage と同じ 480px 幅縮小（CIImage スケール）。
    private func downscaleTo480(_ cg: CGImage) -> CGImage {
